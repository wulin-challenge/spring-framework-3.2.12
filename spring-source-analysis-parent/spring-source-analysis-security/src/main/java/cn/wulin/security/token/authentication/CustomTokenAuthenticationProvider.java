package cn.wulin.security.token.authentication;
...
public class CustomTokenAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    public CustomTokenAuthenticationProvider() {
    ...
    protected void additionalAuthenticationChecks(UserDetails userDetails,
            CustomTokenUsernamePasswordAuthenticationToken authentication) throws AuthenticationException {
        Object salt = null;
    ...
    protected final UserDetails retrieveUser(String username, CustomTokenUsernamePasswordAuthenticationToken authentication)
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        Assert.isInstanceOf(CustomTokenUsernamePasswordAuthenticationToken.class, authentication,
            messages.getMessage("AbstractUserDetailsAuthenticationProvider.onlySupports",
                "Only CustomTokenUsernamePasswordAuthenticationToken is supported"));
        ...
        if (user == null) {
            ...
            try {
                user = retrieveUser(username, (CustomTokenUsernamePasswordAuthenticationToken) authentication);
            } catch (UsernameNotFoundException notFound) {
        ...
        try {
            ...
            additionalAuthenticationChecks(user, (CustomTokenUsernamePasswordAuthenticationToken) authentication);
	...
	protected Authentication createSuccessAuthentication(Object principal, Authentication authentication,
		...
		List<GrantedAuthority> grantedAuthorityList = new ArrayList<GrantedAuthority>(user.getAuthorities());
		CustomTokenUsernamePasswordAuthenticationToken token = 
		new CustomTokenUsernamePasswordAuthenticationToken(principal, authentication.getCredentials(),grantedAuthorityList);
		
		Long currentTime = System.currentTimeMillis();
		token.setToken(UUID.randomUUID().toString());
		token.setCreateTime(currentTime);
		token.setIsExpire(false);
		token.setExpireSecond(30);
		token.setExpireFreshTime(currentTime);
	...
	public boolean supports(Class<?> authentication) {
		return (CustomTokenUsernamePasswordAuthenticationToken.class.isAssignableFrom(authentication));
	}

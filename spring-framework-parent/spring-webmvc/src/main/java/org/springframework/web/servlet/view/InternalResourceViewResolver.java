 ...
 * 
 * <p> UrlBasedViewResolver的便捷子类，支持InternalResourceView（即Servlet和JSP）和子类，如JstlView。
 *
 ...
 * JSTL API is present.
 * 
 * <p> 通过这个解析器生成所有的视图,这个视图类可以通过{@link #setViewClass}指定,更多详细请见{@link UrlBasedViewResolver}
 * 文档,默认是{@link InternalResourceView}或者{@link JstlView}(如果JSTL API 存在)
 *
 ...
 * 
 * <p> 顺便说一句，将JSP文件作为视图放在WEB-INF下是一个很好的实践，以避免直接访问它们（例如通过手动输入的URL）。只有控制器才能访问它们。
 *
 ...
 * 
 * <p> 注意：链接ViewResolvers时，InternalResourceViewResolver始终需要为last，

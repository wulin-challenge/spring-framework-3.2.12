 ...
 * 
 * <p> 一个了解模板的表达式解析器。 它可以通过不提供模板的第一类支持的表达式解析器进行子类化。
 *
 * @author Keith Donald
...
public abstract class TemplateAwareExpressionParser implements ExpressionParser {
	 ...
	 * 
	 * <p> Helper使用配置的解析器解析给定的表达式字符串。 表达式字符串可以包含“$ {...}”标记中包含的任意数量的表达式。 
	 * 例如：“foo $ {expr0} bar $ {expr1}”。 静态文本片段也将作为表达式返回，只返回静态文本片段。 
	 * 因此，评估所有返回的表达式并连接结果将生成完整的计算字符串。 只有最外层的分隔符才能解包，
	 * 所以字符串'hello $ {foo $ {abc}}'会分成'hello'和'foo $ {abc}'。 这意味着支持使用$ {..}作为其功能的一部分的表达式语言没有任何问题。 
	 * 解析意识到嵌入式表达式的结构。 它假定括号'（'，方括号'['和大括号'}'必须在表达式中成对出现，除非它们在字符串文字中，并且字符串文字以单引号开始和终止'。
	 *
	 * @param expressionString the expression string - 表达式字符串
	 * @return the parsed expressions - 解析的表达式
	 * @throws ParseException when the expressions cannot be parsed - 当表达式无法解析时
	 */

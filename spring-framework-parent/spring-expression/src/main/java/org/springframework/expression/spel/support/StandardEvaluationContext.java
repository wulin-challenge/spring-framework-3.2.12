...
public class StandardEvaluationContext implements EvaluationContext {
	 ...
	 * 
	 * <p> 注册一个MethodFilter，它将在方法解析期间为指定的类型调用。
	 * 
	 ...
	 * 
	 * <p> MethodFilter可以移除方法和/或对方法进行排序，然后SpEL将使用这些方法作为查看匹配的候选者。
	 * 
	 * @param type the type for which the filter should be called - 应该调用过滤器的类型
	 * @param filter a {@code MethodFilter}, or {@code null} to unregister a filter for the type
	 * 
	 * <p> 一个MethodFilter，或取消注册该类型的过滤器为null
	 * 
	 * @throws IllegalStateException if the {@link ReflectiveMethodResolver} is not in use
	 * 
	 * <p> 如果ReflectiveMethodResolver未使用
	 * 
	 */

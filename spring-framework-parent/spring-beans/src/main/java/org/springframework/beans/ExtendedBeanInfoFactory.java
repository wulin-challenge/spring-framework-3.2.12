 ...
 * 
 * <p> BeanInfoFactory实现，用于评估bean类是否具有“非标准”JavaBeans setter方法，因此可以通过Spring（包
 * 可见）ExtendedBeanInfo实现进行内省。
 *
 * <p>Ordered at {@link Ordered#LOWEST_PRECEDENCE} to allow other user-defined
 * {@link BeanInfoFactory} types to take precedence.
 * 
 * <p> 在Ordered.LOWEST_PRECEDENCE处订购，以允许其他用户定义的BeanInfoFactory类型优先。
 *
 * @author Chris Beams
 * @since 3.2
 * @see BeanInfoFactory
...
public class ExtendedBeanInfoFactory implements BeanInfoFactory, Ordered {
	 ...
	 * 
	 * <p> 如果适用，返回给定bean类的{@link ExtendedBeanInfo}。
	 * 
	 */
	 ...
	 * 
	 * <p> 返回给定的bean类是声明还是继承任何非void返回bean属性或索引属性setter方法。
	 */

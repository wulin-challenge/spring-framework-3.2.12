 ...
 * 
 * <p> 适用于给定的PropertyEditorRegistrySupport实例。 由BeanWrapperImpl和SimpleTypeConverter用作委托。
 *
...
class TypeConverterDelegate {
	 ...
	 * 
	 * <p> 为给定的编辑器注册表创建一个新的TypeConverterDelegate。
	 * 
	 ...
	 * <p> 将值转换为指定的必需类型。
	 * 
	 * @param newValue the proposed new value - 拟议的新值
	 * @param requiredType the type we must convert to
	 * (or {@code null} if not known, for example in case of a collection element)
	 * 
	 * <p> 我们必须转换为的类型（如果不知道则为null，例如在集合元素的情况下）
	 * 
	 * @param methodParam the method parameter that is the target of the conversion
	 * (may be {@code null})
	 * 
	 * <p> 作为转换目标的方法参数（可以为null）
	 * 
	 ...
	 * 
	 * <p> 使用给定的属性编辑器将值转换为所需的类型（如果需要，从String）。
	 * 
	 * @param oldValue the previous value, if available (may be {@code null})
	 * 
	 * <p> 前一个值（如果可用）（可以为null）
	 * 
	 * @param newValue the proposed new value - 拟定一个新值
	 * @param requiredType the type we must convert to
	 * (or {@code null} if not known, for example in case of a collection element)
	 * 
	 * <p> 我们必须转换为的类型（如果不知道则为null，例如在集合元素的情况下）
	 * 
	 * @param editor the PropertyEditor to use - 要使用的PropertyEditor
	 * @return the new value, possibly the result of type conversion - 新值，可能是类型转换的结果
	 * @throws IllegalArgumentException if type conversion failed - 如果类型转换失败
	 */
	 ...
	 * 
	 * <p> 使用给定的属性编辑器转换给定的文本值。
	 * 
	 * @param oldValue the previous value, if available (may be {@code null}) 
	 * 
	 * <p> 前一个值（如果可用）（可以为null）
	 * 
	 * @param newTextValue the proposed text value - 建议的文本值
	 * @param editor the PropertyEditor to use - 要使用的PropertyEditor
	 * @return the converted value - 转换后的价值
	 */

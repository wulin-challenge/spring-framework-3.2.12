...
public abstract class BeanUtils {
	 ...
	 * class-loading issues.
	 * 
	 * <p>使用no-arg构造函数实例化一个类。 由于此方法不尝试按名称加载类，因此应避免类加载问题。
	 * 
	 * <p>Note that this method tries to set the constructor accessible
	 * if given a non-accessible (that is, non-public) constructor.
	 * 
	 * <p>请注意，如果给定不可访问（即非公共）构造函数，此方法会尝试设置构造函数。
	 * 
	 * @param clazz class to instantiate - 要实例化的类
	 * @return the new instance - 新的实例
	 * @throws BeanInstantiationException if the bean cannot be instantiated 如果bean无法实例化
	 */
	public static <T> T instantiateClass(Class<T> clazz) throws BeanInstantiationException {
		...
		try {
			//返回一个Constructor对象，该对象反映此Class对象所表示的类或接口的指定构造函数。 parameterTypes参数是一个Class对象数组，
			//它按声明的顺序标识构造函数的形式参数类型。 如果此Class对象表示在非静态上下文中声明的内部类，则形式参数类型包括显式封闭实例作为第一个参数。
	 ...
	 * class-loading issues.
	 * 
	 * <p>使用给定构造函数实例化类的便捷方法。 由于此方法不尝试按名称加载类，因此应避免类加载问题。
	 * 
	 * <p>Note that this method tries to set the constructor accessible
	 * if given a non-accessible (that is, non-public) constructor.
	 * 

 ...
 * 
 * <p> 用于活动bean视图的适配器，从本地ApplicationContext（使用本地LiveBeansView bean定义）或所有已注册的
 * ApplicationContexts（由“spring.liveBeansView.mbeanDomain”环境属性驱动）构建当前bean及其依赖项的快照。
 *
 * <p>Note: This feature is still in beta and primarily designed for use with
 * Spring Tool Suite 3.1 and higher.
 * 
 * <p> 注意：此功能仍处于测试阶段，主要用于Spring Tool Suite 3.1及更高版本。
 *
...
public class LiveBeansView implements LiveBeansViewMBean, ApplicationContextAware {
	 ...
	 * 
	 * <p> 生成当前bean及其依赖项的JSON快照，通过findApplicationContexts（）查找所有活动的
	 * ApplicationContexts，然后委托给generateJson（java.util.Set）。
	 * 
	 */
	 ...
	 * 
	 * <p> 查找当前应用程序的所有适用ApplicationContexts。
	 * 
	 ...
	 * 
	 * @return the set of ApplicationContexts - ApplicationContexts的集合
	 */
	 ...
	 * 
	 * <p> 实际上，在给定的ApplicationContexts中生成bean的JSON快照。
	 * 
	 ...
	 * 
	 * <p> 此实现不使用任何JSON解析库以避免第三方库依赖性。 它生成一组上下文描述对象，每个对象包含一个context和
	 * parent属性，以及一个带有嵌套bean描述对象的beans属性。 每个bean对象都包含一个bean，作用域，类型和资源属性，
	 * 以及一个依赖属性，该属性具有当前bean所依赖的bean名称的嵌套数组。
	 * 
	 * @param contexts the set of ApplicationContexts - ApplicationContexts的集合
	 * @return the JSON document - JSON文档
	 */
	 ...
	 * 
	 * <p> 确定指定的bean是否有资格包含在LiveBeansView JSON快照中。
	 * 
	 * @param beanName the name of the bean - bean的名字
	 * @param bd the corresponding bean definition - 相应的bean定义
	 * @param bf the containing bean factory - 包含bean的工厂
	 * @return {@code true} if the bean is to be included; {@code false} otherwise
	 * 
	 * <p> 如果要包含bean，则为true; 否则是假的
	 * 
	 */
	 ...
	 * 
	 * <p> 确定给定bean定义的资源描述，并对其应用基本的JSON转义（反斜杠，双引号）。
	 * 
	 * @param bd the bean definition to build the resource description for
	 * 
	 * <p> 用于构建资源描述的bean定义
	 * 
	 * @return the JSON-escaped resource description - JSON转义的资源描述
	 */

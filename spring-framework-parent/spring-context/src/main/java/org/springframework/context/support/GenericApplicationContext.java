 ...
 * org.springframework.beans.factory.support.DefaultListableBeanFactory实例，并且不假定特定的bean定义格式。 实现
 * org.springframework.beans.factory.support.BeanDefinitionRegistry接口，以允许将任何bean定义读取器应用于它。
 *
 ...
 * etc).
 *
 * <p> 典型用法是通过org.springframework.beans.factory.support.BeanDefinitionRegistry
 * 接口注册各种bean定义，然后调用refresh（）以使用应用程序上下文语义初始化这些bean（处理
 * org.springframework.context.ApplicationContextAware，auto- 检测BeanFactoryPostProcessors等）。
 * 
 ...
 * 它们更容易设置 - 但不太灵活，因为您可以只使用XML bean定义的标准资源位置，而不是混合任意bean定义格式。 
 ...
 * 
 * <p> 对于应该以可刷新的方式读取特殊bean定义格式的自定义应用程序上下文实现，请考虑从
 * AbstractRefreshableApplicationContext基类派生。
 *

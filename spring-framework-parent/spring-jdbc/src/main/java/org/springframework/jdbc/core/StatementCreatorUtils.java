 ...
 * for LOB values).
 * 
 * <p> PreparedStatementSetter / Creator和CallableStatementCreator实现的实用方法，
...
public abstract class StatementCreatorUtils {
	 ...
	 * 
	 * <p> 指示Spring完全忽略java.sql.ParameterMetaData.getParameterType的系统属性，
	 * 即甚至从未尝试为StatementCreatorUtils.setNull调用检索PreparedStatement.getParameterMetaData（）。
	 * 
	 ...
	 * 
	 * <p> 默认值为“false”，首先尝试getParameterType调用，然后根据公共数据库的众所周知的行为回退到
	 * PreparedStatement.setNull / PreparedStatement.setObject调用。 
	 * Spring使用不起作用的getParameterType实现记录JDBC驱动程序，并且不会再次尝试为该驱动程序调用该方法，总是会退回。
	 * 
	 ...
	 * 
	 * <p> 如果您在运行时遇到错误行为，请考虑将此标志切换为“true”。 如果从getParameterType抛出异常（在JBoss AS 7上报告），
	 * 则使用连接池设置PreparedStatement实例。
	 */
	...
	// 使用ConcurrentHashMap作为集合（兼容Java 5）
	 ...
	 * 
	 * @param javaType the Java type to translate - 要翻译的Java类型
	 * 
	 * @return the corresponding SQL type, or {@code null} if none found
	 * 
	 * <p> 相应的SQL类型，如果没有找到则返回null
	 * 
	 */
	 ...
	 * 
	 * <p> 设置参数的值。 使用的方法基于参数的SQL类型，我们可以处理复杂类型，如数组和LOB。
	 * 
	 ...
	 * <p> 声明的参数包括类型
	 * 
	 * @param inValue the value to set - 要设置的值
	 * 
	 * @throws SQLException if thrown by PreparedStatement methods
	 * 
	 * <p> 如果由PreparedStatement方法抛出
	 * 
	 */
	 ...
	 * <p> 我们正在设置的参数的索引
	 * 
	 * @param sqlType the SQL type of the parameter - 参数的SQL类型
	 * 
	 * @param inValue the value to set (plain value or a SqlTypeValue)
	 * 
	 * <p> 要设置的值（普通值或SqlTypeValue）
	 * 
	 * @throws SQLException if thrown by PreparedStatement methods
	 * 
	 * <p> 如果由PreparedStatement方法抛出
	 ...
	 * 
	 * <p> 参数的类型名称（可选，仅用于SQL NULL和SqlTypeValue）
	 * 
	 * @param inValue the value to set (plain value or a SqlTypeValue)
	 * 
	 * <p> 要设置的值（普通值或SqlTypeValue）
	 ...
	 * 
	 * <p> 小数点后的位数（对于DECIMAL和NUMERIC类型）
	 * 
	 * @param inValue the value to set (plain value or a SqlTypeValue)
	 * 
	 * <p> 要设置的值（普通值或SqlTypeValue）抛出：
	 * 
	 * @throws SQLException if thrown by PreparedStatement methods
	 * 
	 * <p> 如果由PreparedStatement方法抛出
	 ...
	 * 
	 * <p> 将指定的PreparedStatement参数设置为null，尊重特定于数据库的特性。
	 */
	private static void setNull(PreparedStatement ps, int paramIndex, int sqlType, String typeName) throws SQLException {
		if (sqlType == SqlTypeValue.TYPE_UNKNOWN) {
			...
			if (sqlTypeToUse == null) {
				...
				// JDBC驱动程序不符合JDBC 3.0  - >继续进行特定于数据库的检查
	 ...
	 * 
	 * <p> 检查给定值是否为java.util.Date（但不是JDBC特定的子类之一）。
	 * 
	 */
	 ...
	 * 
	 * <p> 清理传递给execute方法的参数值所持有的所有资源。 例如，这对于关闭LOB值很重要。
	 * 

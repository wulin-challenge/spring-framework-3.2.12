 ...
 * 
 * <p> 接口，指定一组基本的JDBC操作。 由JdbcTemplate实现。 不经常直接使用，但它是一个增强可测试性的有用选项，
 * 因为它很容易被模拟或存根。
 *
 ...
 * 
 * <p> 或者，可以模拟标准JDBC基础结构。 但是，模仿这个界面构成的工作要少得多。 作为测试数据访问代码的模拟对象方法的替代方法，
 * 请考虑在spring-test.jar中提供的org.springframework.test包中提供的强大的集成测试支持。
 *
 * @author Rod Johnson
...
public interface JdbcOperations {
	 ...
	 * 
	 * <p> 执行JDBC数据访问操作，实现为在JDBC连接上进行的回调操作。 这允许在Spring的托管JDBC环境中实现任意数据访问操作：
	 * 即，参与Spring管理的事务并将JDBC SQLExceptions转换为Spring的DataAccessException层次结构。
	 * 
	 ...
	 * <p> 指定操作的回调对象
	 * 
	 * @throws DataAccessException if there is any problem - 如果有任何问题
	 */
	 ...
	 * 
	 * <p> 执行JDBC数据访问操作，实现为处理JDBC语句的回调操作。 这允许在Spring的托管JDBC环境中对单个
	 * Statement实现任意数据访问操作：即，参与Spring管理的事务并将JDBC SQLExceptions转换为
	 * Spring的DataAccessException层次结构。
	 * 
	 * <p>The callback action can return a result object, for example a
	 * domain object or a collection of domain objects.
	 * 
	 ...
	 * 
	 * <p> 发出单个SQL执行，通常是DDL语句。
	 * 
	 * @param sql static SQL to execute - 要执行的静态SQL
	 * @throws DataAccessException if there is any problem - 如果有任何问题
	 */
	 ...
	 * ResultSetExtractor.
	 * 
	 * <p> 在给定静态SQL的情况下执行查询，使用ResultSetExtractor读取ResultSet。
	 * 
	 ...
	 * 请使用带有null作为参数数组的重载查询方法。
	 * 
	 * @param sql SQL query to execute - 要执行的SQL查询
	 ...
	 * 
	 * <p> 执行给定静态SQL的查询，使用RowCallbackHandler以每行为基础读取ResultSet。
	 * 
	 * <p>Uses a JDBC Statement, not a PreparedStatement. If you want to
	 * execute a static query with a PreparedStatement, use the overloaded
	 * {@code query} method with {@code null} as argument array.
	 ...
	 * via a RowMapper.
	 * 
	 * <p> 执行给定静态SQL的查询，通过RowMapper将每一行映射到Java对象。
	 * 
	 * <p>Uses a JDBC Statement, not a PreparedStatement. If you want to
	 * execute a static query with a PreparedStatement, use the overloaded
	 * {@code query} method with {@code null} as argument array.
	 ...
	 * 
	 * <p> 在给定静态SQL的情况下执行查询，通过RowMapper将单个结果行映射到Java对象。
	 * 
	 * <p>Uses a JDBC Statement, not a PreparedStatement. If you want to
	 * execute a static query with a PreparedStatement, use the overloaded
	 ...
	 * 
	 * <p> 使用JDBC语句，而不是PreparedStatement。 如果要使用PreparedStatement执行静态查询，
	 * 请使用重载的queryForObject（String，RowMapper，Object）方法，并将null作为参数数组。
	 * 
	 * @param sql SQL query to execute - 要执行的SQL查询
	 * 
	 * @param rowMapper object that will map one object per row - 将每行映射一个对象的对象
	 * @return the single mapped object - 单个映射对象
	 ...
	 * 
	 * <p> 使用JDBC语句，而不是PreparedStatement。 如果要使用PreparedStatement执行静态查询，
	 * 请使用重载的queryForObject（String，Class，Object）方法，并将null作为参数数组。
	 * 
	 * <p>This method is useful for running static SQL with a known outcome.
	 * The query is expected to be a single row/single column query; the returned
	 * result will be directly mapped to the corresponding object type.
	 * 
	 * <p> 此方法对于运行具有已知结果的静态SQL非常有用。 该查询应该是单行/单列查询; 返回的结果将直接映射到相应的对象类型。
	 * 
	 * @param sql SQL query to execute - 要执行的SQL查询
	 * 
	 ...
	 * 
	 * <p> 必需类型的结果对象，如果是SQL NULL，则返回null
	 * 
	 * @throws IncorrectResultSizeDataAccessException if the query does not return
	 * exactly one row, or does not return exactly one column in that row
	 * 
	 * <p> 如果查询没有返回一行，或者没有返回该行中的一列
	 * 
	 * @throws DataAccessException if there is any problem executing the query
	 * 
	 * <p> 如果执行查询有任何问题
	 ...
	 * 
	 * <p> 使用JDBC语句，而不是PreparedStatement。 如果要使用PreparedStatement执行静态查询，
	 * 请使用重载的queryForMap（String，Object）方法，并将null作为参数数组。
	 * 

 ...
 * 
 * <p> RowMapper实现，用于将单个列转换为每行的单个结果值。 期望在仅包含单个列的java.sql.ResultSet上运行。
 *
 ...
 * 
 * <p> 可以指定每行的结果值的类型。 单个列的值将从ResultSet中提取并转换为指定的目标类型。
 *
...
public class SingleColumnRowMapper<T> implements RowMapper<T> {
	 ...
	 * 
	 * <p> 验证只选择了一列，然后根据需要委托给getColumnValue（）和convertValueToRequiredType。
	 * 
	...
	public T mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		/**
		 * 检索此ResultSet对象列的数量，类型和属性。
		 * @return 此ResultSet对象的列的说明
		 */
		ResultSetMetaData rsmd = rs.getMetaData();//验证返回结果
	 ...
	 * 
	 * <p> 将给定列值转换为指定的必需类型。 仅在提取的列值不匹配时才调用。
	 * 
	 ...
	 * 
	 * <p> 如果所需类型是String，则该值将通过toString（）简单地进行字符串化。 如果是Number，
	 * 则该值将通过数字转换或字符串解析（取决于值类型）转换为数字。
	 * 
	 * @param value the column value as extracted from {@code getColumnValue()}
	 * (never {@code null})
	 * 
	 * <p> 从getColumnValue（）中提取的列值（从不为null）
	 * 
	 * @param requiredType the type that each result object is expected to match
	 * (never {@code null})
	 * 
	 * <p>  the type that each result object is expected to match (never null)
	 * 

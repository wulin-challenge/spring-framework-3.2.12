...
public abstract class ReflectionUtils {
	 ...
	 * 
	 * <p> 确定给定方法是否显式声明给定的异常或其超类之一，这意味着该类型的异常可以在反射调用中按原样传播。
	 * 
	 * @param method the declaring method - 声明方法
	 * 
	 * @param exceptionType the exception to throw - 要抛出的异常
	 * 
	 * @return {@code true} if the exception can be thrown as-is;
	 * {@code false} if it needs to be wrapped
	 * 
	 * <p> 如果异常可以按原样抛出，则返回true; 如果需要包装，则为false
	 * 
	 */
	 ...
	 * 
	 * <p>确定给定字段是否为“公共静态最终”常量。
	 * @param field the field to check  要检查的字段
	 */
	 ...
	 * 
	 * <p> 确定给定方法是否最初由java.lang.Object声明。
	 * 
	 */
	 ...
	 * 
	 * <p> 确定给定方法是否是CGLIB'重命名'方法，遵循模式“CGLIB $ methodName $ 0”。
	 * 
	 ...
	 * 
	 * <p> 使给定字段可访问，必要时明确设置它可访问。 只有在实际需要时才会调用setAccessible（true）方法，
	 ...
	 * SecurityManager (if active).
	 * 
	 * <p> 使给定方法可访问，必要时明确设置它可访问。 只有在实际需要时才会调用setAccessible（true）方法，
	 * 以避免与JVM SecurityManager（如果处于活动状态）发生不必要的冲突。
	 * 
	 * @param method the method to make accessible - 使访问的方法
	 ...
	 * SecurityManager (if active).
	 * 
	 * <p>使给定的构造函数可访问，必要时显式设置它是可访问的。 只有在实际需要时才会调用setAccessible（true）方法，
	 * 以避免与JVM SecurityManager（如果处于活动状态）发生不必要的冲突。
	 * 
	 * @param ctor the constructor to make accessible - 使构造函数可访问

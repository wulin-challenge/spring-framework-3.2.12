 ...
 * 
 * <p> 一般实用程序方法，用于处理注释，处理桥接方法（编译器为通用声明生成）以及超级方法（用于可选的“注释继承”）。 请注意，这些都不是由JDK的内省设施本身提供的。
 *
 ...
 * 
 * <p> 作为运行时保留注释的一般规则（例如，用于事务控制，授权或服务公开），始终在此类上使用查找方法（例如，findAnnotation（Method，Class），
 * getAnnotation（Method，Class）和getAnnotations（Method） ））而不是JDK中的普通注释查找方法。 
 * 您仍然可以在给定类级别上查找（getAnnotation（Method，Class））和在给定方法的整个继承层次结构中查找（findAnnotation（Method，Class））。
 *
 * @author Rob Harrop
...
public abstract class AnnotationUtils {
	 ...
	 * 
	 * <p> 从提供的Method，Constructor或Field中获取annotationType的单个Annotation。 
	 * 如果未在提供的元素上本地声明注释，则将搜索元注释。
	 * 
	 * @param ae the Method, Constructor or Field from which to get the annotation
	 * 
	 * <p> 从中获取注释的Method，Constructor或Field
	 * 
	 ...
	 * 
	 * <p> 如果在给定的类本身上找不到注释，则从提供的Class中查找annotation注释类型，遍历其接口和超类。
	 * 
	 ...
	 * 
	 * <p> 该算法的操作如下：在给定的类上搜索注释并在找到时返回它。 否则搜索给定类声明的所有接口，从第一个匹配候选项返回注释（如果有）。 
	 * 否则继续对给定类的超类进行内省，检查超类本身; 如果没有找到注释，则继续使用超类声明的接口。 如果未找到匹配项，则递归整个超类层次结构。
	 * 
	 * @param clazz the class to look for annotations on - 要查找注释的类
	 * @param annotationType the annotation type to look for - 要查找的注释类型
	 * @return the annotation found, or {@code null} if none found - 找到的注释，如果没有找到则为null
	 */
